  ...
  return (
    <article
      className={`rounded-card border p-4 ${
        expert ? "border-marea/40 bg-[#103f41]" : "border-line bg-surface"
      }`}
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          {expert && (
            <span className="mt-2 inline-block rounded bg-coral px-2 py-0.5 font-mono text-[9px] uppercase tracking-wide text-white">
              solo expertos
            </span>
          )}
        </div>
      ...
      <p className="mt-3 font-mono text-[10.5px] text-foam-dim">
        mejor ventana: <span className="text-coral">{props.window}</span>
      </p>
    </article>
  );
}
...
function Readout({ k, v, u }: { k: string; v: string; u: string }) {
  return (
    <div>
      ...
      <div className="mt-0.5 text-[15px] text-foam">
        {v}
        <span className="text-[9px] text-foam-faint"> {u}</span>
      </div>
    </div>
  );
}
